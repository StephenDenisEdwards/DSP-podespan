...
Cut line runs through the centre of the pond (perpendicular to separation wall),
from the terrace/deck through the swimming zone, separation wall, and all three
regeneration depth zones.

Matches dimensions from create_schwimmteich_3d.py.
"""
...
# Regen zone depths (3 zones)
Z1_Z = -0.15    # marginal zone floor (0-30cm, avg ~15cm)
Z2_Z = -0.45    # shallow zone floor (30-60cm, avg ~45cm)
Z3_Z = -1.00    # deep gravel bed floor (60-100cm)

# Zone widths (from regen zone edge inward)
Z1_W = 0.5
Z2_W = 0.5
...
# =============================================================
ground_xs = [DECK_START - 1.0, POND_END + 1.5, POND_END + 1.5, DECK_START - 1.0]
...
ax.plot([POND_END, POND_END + 1.5], [TERRACE_Z - 0.05, TERRACE_Z - 0.5],
        color='#8B7355', lw=2, zorder=1)
...
# Regen zone shell — stepped profile
# Zone 1 floor (near wall side)
regen_start = WALL_POS + WALL_T/2
z1_end = regen_start + Z1_W
z2_end = z1_end + Z2_W
z3_end = POND_END - Z1_W - Z2_W
z3_far_z2 = POND_END - Z1_W
z3_far_z1 = POND_END

# Shell under regen zone (follows stepped profile)
shell_xs = [regen_start, z1_end, z1_end, z2_end, z2_end,
            z3_end, z3_end, z3_far_z2, z3_far_z2, z3_far_z1, z3_far_z1,
            POND_END + SHELL_T, POND_END + SHELL_T,
            regen_start]
shell_zs = [Z1_Z - SHELL_T, Z1_Z - SHELL_T, Z2_Z - SHELL_T, Z2_Z - SHELL_T,
            Z3_Z - SHELL_T, Z3_Z - SHELL_T,
            Z2_Z - SHELL_T, Z2_Z - SHELL_T, Z1_Z - SHELL_T, Z1_Z - SHELL_T,
            TERRACE_Z, TERRACE_Z,
            Z1_Z - SHELL_T,
            Z1_Z - SHELL_T]
# Simplified: just show the shell as a thin line under the profile
# Draw the stepped floor with shell underneath
regen_profile_x = [regen_start, z1_end, z1_end, z2_end, z2_end,
                   z3_end, z3_end, z3_far_z2, z3_far_z2, z3_far_z1]
regen_profile_z = [Z1_Z, Z1_Z, Z2_Z, Z2_Z, Z3_Z,
                   Z3_Z, Z2_Z, Z2_Z, Z1_Z, Z1_Z]
...
for i in range(len(regen_profile_x) - 1):
    ...
    z_bot = z_top - SHELL_T
    poly(ax, [x0, x1, x1, x0], [z_top, regen_profile_z[i+1], regen_profile_z[i+1] - SHELL_T, z_bot],
         shell_color, alpha=0.5, ec='#78909C', lw=0.5, zorder=1)

# Far wall shell
poly(ax, [POND_END, POND_END + SHELL_T, POND_END + SHELL_T, POND_END],
...
# Water in swimming zone
swim_water_xs = [POND_START, WALL_POS - WALL_T/2, WALL_POS - WALL_T/2, POND_START]
swim_water_zs = [WATER_Z, WATER_Z, SWIM_FLOOR_Z, SWIM_FLOOR_Z]

# Subtract steps area — draw water around steps
...
# =============================================================
# 6. REGENERATION ZONE - THREE DEPTH ZONES
# =============================================================

# --- Zone 1: Marginal (0-30cm) ---
# Near side (next to wall)
poly(ax, [regen_start, z1_end, z1_end, regen_start],
         [WATER_Z, WATER_Z, Z1_Z, Z1_Z],
     '#43A047', alpha=0.35, ec='#2E7D32', lw=1.0, zorder=3)
# Far side
poly(ax, [z3_far_z1, POND_END, POND_END, z3_far_z1],
         [WATER_Z, WATER_Z, Z1_Z, Z1_Z],
     '#43A047', alpha=0.35, ec='#2E7D32', lw=1.0, zorder=3)

# Zone 1 floor lines
ax.plot([regen_start, z1_end], [Z1_Z, Z1_Z], color='#2E7D32', lw=2, zorder=4)
ax.plot([z3_far_z1, POND_END], [Z1_Z, Z1_Z], color='#2E7D32', lw=2, zorder=4)

# --- Zone 2: Shallow (30-60cm) ---
# Near side
poly(ax, [z1_end, z2_end, z2_end, z1_end, z1_end],
         [WATER_Z, WATER_Z, Z2_Z, Z2_Z, Z1_Z],
     '#FDD835', alpha=0.35, ec='#F9A825', lw=1.0, zorder=3)
# Far side
poly(ax, [z3_end, z3_far_z2, z3_far_z2, z3_far_z1, z3_far_z1, z3_end],
         [Z2_Z, Z2_Z, WATER_Z, WATER_Z, Z1_Z, Z1_Z],
     '#FDD835', alpha=0.35, ec='#F9A825', lw=1.0, zorder=3)

# Step-down faces
ax.plot([z1_end, z1_end], [Z1_Z, Z2_Z], color='#33691E', lw=2.5, zorder=5)
ax.plot([z3_far_z1, z3_far_z1], [Z1_Z, Z2_Z], color='#33691E', lw=2.5, zorder=5)

# Zone 2 floor lines
ax.plot([z1_end, z2_end], [Z2_Z, Z2_Z], color='#F9A825', lw=2, zorder=4)
ax.plot([z3_end, z3_far_z2], [Z2_Z, Z2_Z], color='#F9A825', lw=2, zorder=4)

# --- Zone 3: Deep gravel bed (60-100cm) ---
# Water above gravel
poly(ax, [z2_end, z3_end, z3_end, z2_end],
         [WATER_Z, WATER_Z, GRAVEL_FINE_Z, GRAVEL_FINE_Z],
     '#4DB6AC', alpha=0.2, ec='none', zorder=3)

# Step-down faces Z2→Z3
ax.plot([z2_end, z2_end], [Z2_Z, Z3_Z], color='#BF360C', lw=2.5, zorder=5)
ax.plot([z3_end, z3_end], [Z2_Z, Z3_Z], color='#BF360C', lw=2.5, zorder=5)

# Zone 3 floor
ax.plot([z2_end, z3_end], [Z3_Z, Z3_Z], color='#5D4037', lw=2, zorder=4)

# =============================================================
# 7. GRAVEL LAYERS (in Zone 3)
# =============================================================

# Coarse gravel (bottom layer)
poly(ax, [z2_end + 0.05, z3_end - 0.05, z3_end - 0.05, z2_end + 0.05],
...
np.random.seed(42)
for _ in range(120):
    gx = np.random.uniform(z2_end + 0.1, z3_end - 0.1)
...
ax.plot([z2_end + 0.3, z3_end - 0.3], [PIPE_Z, PIPE_Z],
...
for px in np.linspace(z2_end + 0.6, z3_end - 0.6, 7):
...
# 10. WATER LEVEL LINE
# =============================================================
ax.plot([POND_START, POND_END], [WATER_Z, WATER_Z],
...
# Zone 1 plants (tall emergent, above water)
for px in [regen_start + 0.25, POND_END - 0.25]:
    draw_plant(ax, px, Z1_Z, 0.45, leaf_size=10)

# Zone 2 plants (medium, partially above water)
for px in [z1_end + 0.25, z3_far_z2 + 0.25]:
    draw_plant(ax, px, Z2_Z, 0.55, leaf_size=9)

# Zone 3 plants (submerged oxygenators + lily pads at surface)
for px in np.linspace(z2_end + 0.5, z3_end - 0.5, 4):
    # Submerged stem
...
# Regen zone
ax.annotate('', xy=(POND_END, dim_z), xytext=(WALL_POS + WALL_T/2, dim_z),
            arrowprops=dict(arrowstyle='<->', color=dim_color, lw=dim_lw), zorder=15)
ax.text((WALL_POS + POND_END)/2, dim_z - 0.1, '5.0m\nRegeneration Zone',
        fontsize=8, fontweight='bold', color=dim_color, ha='center', va='top', zorder=15)

# Total
dim_z2 = -2.8
ax.annotate('', xy=(POND_END, dim_z2), xytext=(POND_START, dim_z2),
            arrowprops=dict(arrowstyle='<->', color=dim_color, lw=1.2), zorder=15)
ax.text((POND_START + POND_END)/2, dim_z2 - 0.1, '10.0m total',
        fontsize=9, fontweight='bold', color=dim_color, ha='center', va='top', zorder=15)

# Zone widths within regen
dim_z3 = Z3_Z - 0.25
# Z1 near
ax.annotate('', xy=(z1_end, dim_z3), xytext=(regen_start, dim_z3),
            arrowprops=dict(arrowstyle='<->', color='#2E7D32', lw=0.8), zorder=15)
ax.text((regen_start + z1_end)/2, dim_z3 - 0.06, '0.5m', fontsize=6,
        color='#2E7D32', ha='center', va='top', zorder=15)
# Z2 near
ax.annotate('', xy=(z2_end, dim_z3), xytext=(z1_end, dim_z3),
            arrowprops=dict(arrowstyle='<->', color='#F57F17', lw=0.8), zorder=15)
ax.text((z1_end + z2_end)/2, dim_z3 - 0.06, '0.5m', fontsize=6,
        color='#F57F17', ha='center', va='top', zorder=15)
# Z3 centre
ax.annotate('', xy=(z3_end, dim_z3), xytext=(z2_end, dim_z3),
            arrowprops=dict(arrowstyle='<->', color='#BF360C', lw=0.8), zorder=15)
ax.text((z2_end + z3_end)/2, dim_z3 - 0.06, f'{z3_end - z2_end:.1f}m', fontsize=6,
...
# Zone labels in regen
ax.text((regen_start + z1_end)/2, (WATER_Z + Z1_Z)/2,
...
# Pipe labels
ax.text((z2_end + z3_end)/2, PIPE_Z - 0.1,
...
plt.tight_layout()
plt.savefig('schwimmteich_cross_section.png', dpi=200, bbox_inches='tight',
            facecolor='white', edgecolor='none')
print('Saved schwimmteich_cross_section.png')
