...
# -- Regeneration zone: THREE-ZONE BANDED LAYOUT (V2) --
# Zones as parallel bands across full width, stepping DOWN from wall:
#   Zone 3 (deepest):    next to wall — gravel bed, 60-100cm
#   Zone 2 (mid):        middle band — shallow, 30-60cm
#   Zone 1 (shallowest): far end — marginal, 0-30cm

Z1_Z = -0.15   # marginal zone floor
Z2_Z = -0.45   # shallow zone floor
# GRAVEL_BOT_Z = -1.00 already defined

z1_color = '#43A047'   # strong green — marginal
z2_color = '#FDD835'   # bright yellow — shallow
z3_color = '#E65100'   # deep orange — deep gravel

# Band lengths (y-direction), maintaining area proportions
Z3_LEN = 2.35   # 47% of regen
Z2_LEN = 1.10   # 22% of regen
Z1_LEN = 1.55   # 31% of regen

# Band y-boundaries (Zone 3 nearest wall, Zone 1 at far end)
z3_y0 = WALL_Y
z3_y1 = WALL_Y + Z3_LEN
z2_y0 = z3_y1
z2_y1 = z2_y0 + Z2_LEN
...
# Right wall — swim zone
draw_face(ax,
    [(POND_W, 0, SWIM_FLOOR_Z), (POND_W, WALL_Y, SWIM_FLOOR_Z),
     (POND_W, WALL_Y, TERRACE_Z), (POND_W, 0, TERRACE_Z)],
    '#90A4AE', alpha=0.5, zorder=3)
# Right wall — regen zone (stepped profile)
draw_face(ax,
    [(POND_W, z3_y0, GRAVEL_BOT_Z), (POND_W, z3_y1, GRAVEL_BOT_Z),
     (POND_W, z3_y1, TERRACE_Z), (POND_W, z3_y0, TERRACE_Z)],
    '#90A4AE', alpha=0.45, zorder=3)
draw_face(ax,
    [(POND_W, z2_y0, Z2_Z), (POND_W, z2_y1, Z2_Z),
     (POND_W, z2_y1, TERRACE_Z), (POND_W, z2_y0, TERRACE_Z)],
    '#90A4AE', alpha=0.45, zorder=3)
draw_face(ax,
    [(POND_W, z1_y0, Z1_Z), (POND_W, z1_y1, Z1_Z),
     (POND_W, z1_y1, TERRACE_Z), (POND_W, z1_y0, TERRACE_Z)],
    '#90A4AE', alpha=0.45, zorder=3)

# Front wall (y = POND_L) — Z1 depth
draw_face(ax,
    [(0, POND_L, Z1_Z), (POND_W, POND_L, Z1_Z),
     (POND_W, POND_L, TERRACE_Z), (0, POND_L, TERRACE_Z)],
    '#78909C', alpha=0.5, zorder=3)

# Left wall — CROSS-SECTION with banded steps
draw_face(ax,
...
draw_face(ax,
    [(0, z3_y0, GRAVEL_BOT_Z), (0, z3_y1, GRAVEL_BOT_Z),
     (0, z3_y1, TERRACE_Z), (0, z3_y0, TERRACE_Z)],
    '#BF360C', alpha=0.4, lw=0.8, zorder=3)
# Z2 band
draw_face(ax,
    [(0, z2_y0, Z2_Z), (0, z2_y1, Z2_Z),
     (0, z2_y1, TERRACE_Z), (0, z2_y0, TERRACE_Z)],
    '#F9A825', alpha=0.4, lw=0.8, zorder=3)
# Z1 band
draw_face(ax,
    [(0, z1_y0, Z1_Z), (0, z1_y1, Z1_Z),
     (0, z1_y1, TERRACE_Z), (0, z1_y0, TERRACE_Z)],
    '#43A047', alpha=0.35, lw=0.8, zorder=3)
...
z2_label = iso_project(POND_W + 2.5, z2_y0 + 0.3, Z2_Z + 0.15)
ax.annotate(f'ZONE 2: SHALLOW\n30–60cm  |  {z2_area:.0f} m² ({z2_area/regen_total*100:.0f}%)\nIris, Acorus, Mentha aquatica',
...
z1_label = iso_project(POND_W + 2.5, z1_y0 + 0.5, Z1_Z + 0.3)
ax.annotate(f'ZONE 1: MARGINAL\n0–30cm  |  {z1_area:.0f} m² ({z1_area/regen_total*100:.0f}%)\nCaltha, Juncus, Myosotis',
...
ax.annotate('0.15m', dm1, fontsize=6, fontweight='bold', color='#2E7D32',
            ha='right', va='center', zorder=20, bbox=dict(facecolor='white', edgecolor='none', alpha=0.8, pad=1))

dd_z3_top = iso_project(-0.6, WALL_Y + 0.5, TERRACE_Z)
dd_z3_bot = iso_project(-0.6, WALL_Y + 0.5, GRAVEL_BOT_Z)
ax.annotate('', xy=dd_z3_bot, xytext=dd_z3_top,
            arrowprops=dict(arrowstyle='<->', color='#BF360C', lw=1.2), zorder=20)
dm3 = iso_project(-1.3, WALL_Y + 0.5, (TERRACE_Z + GRAVEL_BOT_Z)/2)
ax.annotate('1.0m', dm3, fontsize=6, fontweight='bold', color='#BF360C',
            ha='right', va='center', zorder=20, bbox=dict(facecolor='white', edgecolor='none', alpha=0.8, pad=1))
...
manifold_y = WALL_Y + Z3_LEN * 0.7  # within Zone 3 band
